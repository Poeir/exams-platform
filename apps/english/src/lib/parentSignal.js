// Notify the embedding parent site (empeo) that the exam finished, carrying the
// server-scored result so the parent can persist its own copy — used when the
// app runs inside an <iframe> the parent embeds, in place of the callback_url
// webhook. The result here is what the server returned from submit (already
// scored server-side); the iframe only relays it.
//

...
const paths = {
  ...
  '/api/sessions': {
    post: {
      ...
      description: 'Creates a pending attempt + one-time launch_token. Returns the launch_url the parent opens in the candidate\'s browser.',
      ...
      requestBody: jsonBody({
        ...
        properties: {
          ...
          external_user_id: {
            type: 'string',
            nullable: true,
            description: 'Candidate id in the parent system. Send the same source_system + external_user_id to every assessment engine so one person maps to one shared subject. user_ref is accepted as a legacy alias.',
          },
          source_system: { type: 'string', nullable: true, default: 'english' },
          email: { type: 'string', nullable: true, description: 'Snapshot stored on the shared subject.' },
          display_name: { type: 'string', nullable: true, description: 'Snapshot stored on the shared subject.' },
          user_ref: { type: 'string', nullable: true, deprecated: true, description: 'Legacy alias for external_user_id.' },
      ...
      responses: {
        201: jsonResp({
          ...
          properties: {
            ...
            launch_token_expires_at: { type: 'string', format: 'date-time' },
            launch_url: { type: 'string', format: 'uri' },
          },
        }),
        400: errorResp('paper_id is required'),
  ...
  '/api/attempts/{id}/view-link': {
    post: {
      ...
      description: 'Exchanges the parent API key for a signed, short-lived URL (default 15 min, VIEW_LINK_TTL_MIN) that opens a read-only result page in a browser — the "open result in a new tab" flow. The token is stateless (HMAC over attempt id + expiry). Only submitted attempts can be viewed.',
      security: [{ parentApiKey: [] }],
      parameters: [{ in: 'path', name: 'id', required: true, schema: { type: 'string' } }],
      responses: {
        201: jsonResp({
        ...
        401: errorResp('Invalid or missing X-API-Key'),
        404: errorResp('No submitted attempt with this id'),
      },
    },
  },
  ...
  '/api/attempts/view': {
    get: {
      ...
      description: 'Browser-facing endpoint behind the view link. Returns a display summary only — no answer key and no raw answer map.',
      parameters: [{ in: 'query', name: 'vt', required: true, schema: { type: 'string' } }],
      responses: {
        200: jsonResp({
          type: 'object',
          properties: {
            ...
            correct_total: { type: 'integer', nullable: true },
            max_total: { type: 'integer', nullable: true },
            cefr_level: { type: 'integer', nullable: true },
            cefr_label: { type: 'string', nullable: true },
            result: { type: 'object', nullable: true },
          },
        }),
        401: errorResp('View token is invalid or has expired'),
        404: errorResp('Submitted attempt not found'),
      },
    },
  },
  ...
  '/api/subjects/{source_system}/{external_user_id}/results': {
    get: {
      ...
      description: "Mirrors the mbti engine's GET /api/v1/subjects/{sourceSystem}/{externalUserId}/results so parents query both engines symmetrically. Submitted attempts only, newest first.",
      security: [{ parentApiKey: [] }],
      parameters: [
      ...
      responses: {
        200: jsonResp({
          type: 'object',
          properties: { results: { type: 'array', items: r('Attempt') } },
        }),
        401: errorResp('Invalid or missing X-API-Key'),
      },
    },
  ...
  '/api/subjects/{source_system}/{external_user_id}/view-link': {
    post: {
      tags: ['Attempts'],
      summary: "Mint a view link for the subject's latest submitted result",
      description: 'Attempt-id-free variant of /api/attempts/{id}/view-link: resolves the latest submitted attempt for (source_system, external_user_id) and mints a short-lived signed view URL for it.',
      security: [{ parentApiKey: [] }],
      parameters: [
        { in: 'path', name: 'source_system', required: true, schema: { type: 'string' } },
        { in: 'path', name: 'external_user_id', required: true, schema: { type: 'string' } },
      ...
      responses: {
        ...
        401: errorResp('Invalid or missing X-API-Key'),
        404: errorResp('No submitted attempt for this subject'),
      },
    },
  },
  ...
  '/api/attempts': {
    get: {
      ...
      responses: {
        200: jsonResp({ type: 'array', items: r('Attempt') }),
        401: errorResp('Invalid or missing X-API-Key'),
      },
    },
  },
